"""
Models das tabelas do SIGCON-MG — o núcleo do domínio de convênios.

Convenio é a tabela principal do painel (consolidada por carregar_convenios);
as demais são as auxiliares, uma por tabela exportada do SIGCON-MG.
"""
...
class PlanoAplicacao(models.Model):
    ...
    # carimbado no ETL via Geral + CodigoConvenio (mesma rota do CronogramaDesembolso)
    convenio_numero_sequencial_siafi = models.CharField(
        "Código SIAFI", max_length=50, null=True, blank=True,
    )
